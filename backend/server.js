...
io.on("connection", (socket) => {
  ...
  socket.on(
    ...
    ({ blockchainRoomId, buyIn, maxPlayers, creator, txHash }) => {
      const shortId = encodeRoomId(blockchainRoomId);
      console.log(
        `Creating blockchain room: ${formatRoomIdDisplay(
          blockchainRoomId
        )} (${shortId})`
      );
  ...
  socket.on(
    ...
    ({ blockchainRoomId, player, txHash }) => {
      const shortId = encodeRoomId(blockchainRoomId);
      console.log(
        `Joining blockchain room: ${formatRoomIdDisplay(
          blockchainRoomId
        )} (${shortId})`
      );
      ...
      if (existingPlayer) {
        ...
        console.log(
          `${player} reconnected to ${formatRoomIdDisplay(blockchainRoomId)}`
        );
        return;
      }
      ...
      console.log(
        `${player} joined ${formatRoomIdDisplay(
          blockchainRoomId
        )} (tx: ${txHash})`
      );
    }
