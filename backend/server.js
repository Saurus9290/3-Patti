...
io.on('connection', (socket) => {
  ...
  socket.on('startGame', ({ blockchainRoomId, txHash } = {}) => {
    ...
    game.players.forEach(player => {
      const playerSocket = Array.from(playerSockets.entries())
        .find(([_, info]) => info.playerId === player.id);
  ...
  socket.on('playerAction', ({ action, amount }) => {
    ...
    if (winner) {
      ...
      console.log(`Game ended in room ${playerInfo.roomId}. Winner: ${winner.name}`);
    } else {
      // Notify whose turn it is
      const currentPlayer = game.getCurrentPlayer();
      io.to(playerInfo.roomId).emit('turnChanged', {
  ...
  socket.on('requestSideshow', ({ targetPlayerId }) => {
    ...
    if (requestingPlayer.isBlind) {
      socket.emit('error', { message: 'Blind players cannot request sideshow' });
      return;
    }

    // Notify target player about sideshow request
    const targetSocket = Array.from(playerSockets.entries())
      .find(([_, info]) => info.playerId === targetPlayerId);
  ...
  socket.on('sideshowResponse', ({ requesterId, accepted }) => {
    ...
    if (accepted) {
      ...
      const requesterSocket = Array.from(playerSockets.entries())
        .find(([_, info]) => info.playerId === requesterId);
